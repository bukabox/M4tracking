...
export function TotalRevenue({ lifetimeRevenue, initialModal = 25000000, onModalChange, roiTarget = 2000 }: TotalRevenueProps) {
  ...
  const [showLifetime, setShowLifetime] = useState<boolean>(true);
  ...
  const roiProgressPct = Math.min(roiPercentage, roiTarget) / roiTarget * 100;
  ...
  return (
    <Card className="p-6 mb-6 bg-white border-gray-200">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <p className="text-gray-500" style={{ fontSize: '0.875rem' }}>Total Revenue (Lifetime)</p>
              <Button 
                variant="ghost" 
                size="icon" 
                className="h-6 w-6"
            ...
            {/* Amount Display */}
            {showLifetime ? (
              <p className="text-gray-900 mb-2" style={{ fontSize: '1.875rem', lineHeight: '2.25rem' }}>
                Rp {fmtIDR(lifetimeRevenue)}
              </p>
            ) : (
              <p className="text-gray-300 mb-2" style={{ fontSize: '1.875rem', lineHeight: '2.25rem' }}>
                ••••••••
              </p>
            )}
            ...
            {showLifetime && (
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 text-gray-600" style={{ fontSize: '0.875rem' }}>
                  <span>dari modal</span>
                  <span className="text-gray-900" style={{ fontWeight: 600 }}>Rp {fmtIDR(modalM4)}</span>
                  <button
        ...
        {showLifetime && (
          <div className="flex-col items-center" style={{ display: window.innerWidth >= 1024 ? 'flex' : 'none' }}>
            <div className="relative" style={{ width: '5rem', height: '5rem' }}>
              {/* Background Circle */}
              <svg style={{ transform: 'rotate(-90deg)', width: '5rem', height: '5rem' }}>
                <circle
                  cx="40"
                  cy="40"
                  r="32"
            ...
            <p className="text-gray-500 mt-1" style={{ fontSize: '0.75rem' }}>Progress</p>
          </div>
        )}
      </div>
      ...
      {showLifetime && (
        <div className="mt-4">
          ...
          <div className="flex items-center justify-between text-gray-500" style={{ marginTop: '0.375rem', fontSize: '0.75rem' }}>
            <span>0%</span>
            <span style={{ fontWeight: 500 }}>Target ROI: {roiTarget}%</span>
            <span>{roiTarget}%</span>
          </div>
        </div>
      )}
      ...
      {isEditingModal && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <p className="text-gray-900 mb-2" style={{ fontSize: '0.875rem', fontWeight: 500 }}>Edit Modal Awal</p>
